"""
Minimum Transaction Threshold — Self-Consistency Test
=====================================================
For each user, at each transaction count N, take 10 different random
subsamples of N transactions, classify each one, and check if they
all agree. This measures: "at N transactions, is the classification
stable regardless of WHICH N transactions we pick?"

No ground truth needed. We're measuring classification reliability.
"""
...
# ─── Category mapping (matches build_sparkov_features.py) ───
CATEGORY_MAP = {
...
def extract_features(g):
    """Extract 15 features — matches build_sparkov_features.py exactly."""
    n_txn = len(g)
    if n_txn < 2:
        return None
...
print(f"Running self-consistency test: {len(user_groups)} users × {len(LEVELS)} levels × {N_SAMPLES} samples\n")
...
for level in LEVELS:
    consistent_users = 0
    tested_users = 0
    majority_agreement = 0  # users where ≥80% of samples agree
    ...
    for uid, txns in user_groups.items():
        ...
        # Check: do ALL samples agree?
        if len(set(personas)) == 1:
            consistent_users += 1

        # Check: do ≥80% agree on one persona?
...
if recommended:
    ...
    print(f"  This is the minimum for reliable classification.")
    if recommended <= 20:
        print(f"  ✅ Your current threshold of 20 is at or above the empirical minimum.")
    else:
        print(f"  ⚠️ Consider raising your full unlock threshold from 20 to {recommended}.")
else:
    print(f"  Consistency never reaches 70% at any tested level.")
    print(f"  The model may need the domain refinement layer to stabilise classifications.")

# ─── Save ───
output = {
    'experiment': 'persona_self_consistency_vs_transaction_count',
    'question': 'At N transactions, if we take 10 different random subsamples, how often do they all produce the same persona?',
